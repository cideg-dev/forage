...
// Fix: Explicitly typing ScrollReveal to avoid children and key property errors in TSX.
      ...
      <div className="max-w-7xl mx-auto px-4">
        <ScrollReveal className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          ...
          <div className="flex gap-4">
            <button onClick={prev} className="p-4 rounded-full border border-white/10 hover:bg-red-600 transition-all shadow-lg active:scale-95" aria-label="Projet précédent"><ChevronLeft /></button>
            <button onClick={next} className="p-4 rounded-full border border-white/10 hover:bg-red-600 transition-all shadow-lg active:scale-95" aria-label="Projet suivant"><ChevronRight /></button>
          </div>
        </ScrollReveal>
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      {/* Navigation Pro */}
      <nav className="fixed w-full z-50 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md py-4 border-b-2 border-red-600 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => scrollTo('accueil')}>
             <div className="bg-cyan-600 p-2 rounded-xl shadow-lg animate-float"><Droplets className="text-white" /></div>
          ...
          <div className="hidden lg:flex items-center gap-8">
             {['Services', 'Projets', 'Contact'].map(item => (
               <button key={item} onClick={() => scrollTo(item.toLowerCase())} className="font-black text-[11px] uppercase tracking-widest hover:text-red-600 transition-colors dark:text-slate-300 active:scale-95">{item}</button>
             ))}
             <button onClick={toggleTheme} className="p-2 rounded-full bg-slate-100 dark:bg-slate-800 dark:text-white hover:rotate-12 transition-transform">
                {isDark ? <Sun size={18} /> : <Moon size={18} />}
             </button>
             <a href={WHATSAPP_LINK} target="_blank" className="bg-red-600 text-white px-8 py-3 rounded-full font-black text-xs uppercase tracking-widest hover:bg-slate-900 transition-all shadow-xl shadow-red-600/20 active:scale-95">Devis Rapide</a>
          </div>
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className="lg:hidden absolute top-full left-0 w-full bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 p-8 flex flex-col gap-6 animate-in slide-in-from-top duration-300">
             {['Services', 'Projets', 'Contact'].map(item => (
               <button key={item} onClick={() => scrollTo(item.toLowerCase())} className="text-left font-black text-2xl uppercase tracking-tighter dark:text-white">{item}</button>
             ))}
             <div className="flex justify-between items-center pt-4 border-t border-slate-100 dark:border-slate-800">
               <button onClick={toggleTheme} className="flex items-center gap-2 font-bold dark:text-white">
                 {isDark ? <Sun size={20} /> : <Moon size={20} />} Mode {isDark ? 'Clair' : 'Sombre'}
               </button>
               <a href={WHATSAPP_LINK} className="bg-red-600 text-white px-6 py-2 rounded-full font-black text-xs uppercase tracking-widest">WhatsApp</a>
             </div>
          </div>
        )}
      </nav>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 w-full">
          <ScrollReveal className="max-w-4xl">
            ...
            <div className="flex flex-wrap gap-6">
              <button onClick={() => scrollTo('services')} className="group bg-slate-950 dark:bg-white text-white dark:text-slate-950 px-12 py-6 rounded-2xl font-black uppercase tracking-widest text-sm transition-all hover:bg-red-600 hover:dark:bg-red-600 hover:text-white shadow-2xl flex items-center gap-4 active:scale-95">
      ...
      <section id="services" className="py-24 bg-white dark:bg-slate-950 scroll-mt-20">
        <div className="max-w-7xl mx-auto px-4">
          <ScrollReveal className="text-center mb-24">
             <div className="bg-red-600 text-white px-6 py-2 rounded-full inline-block font-black text-[10px] uppercase tracking-[0.3em] mb-6 italic shadow-lg shadow-red-600/20">Catalogue Technique</div>
      ...
      {/* Call to Action - Amélioré avec ancres intelligentes */}
      <section className="py-24 bg-gradient-to-r from-cyan-600 to-blue-700 dark:from-cyan-900 dark:to-blue-900 overflow-hidden relative">
         ...
         <ScrollReveal className="max-w-7xl mx-auto px-4 text-center relative z-10">
            ...
            <p className="text-xl text-cyan-50 mb-12 max-w-2xl mx-auto font-medium opacity-90 leading-relaxed">
              Ne laissez pas la sécheresse freiner vos ambitions. Obtenez une étude de terrain scientifique et un devis personnalisé dès aujourd'hui.
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-6">
               <button 
                 ...
                 className="bg-white text-slate-900 px-12 py-6 rounded-2xl font-black uppercase tracking-widest text-sm hover:bg-red-600 hover:text-white transition-all shadow-2xl transform hover:-translate-y-1 active:scale-95"
               >
                 Demander un devis gratuit
               </button>
               <button 
                 onClick={() => handleCTA('Expertise Géophysique', "Besoin d'une recherche scientifique de nappe phréatique pour sécuriser l'investissement.", 'form-name')}
            ...
            <ScrollReveal className="bg-slate-950 text-white p-12 rounded-[4rem] shadow-2xl border border-white/10">
              ...
              <form className="space-y-8" onSubmit={e => { e.preventDefault(); alert('Nos techniciens ont reçu votre demande !'); }}>
                <div className="space-y-2">
                   <label htmlFor="form-name" className="text-[10px] font-black uppercase tracking-widest opacity-60">Votre Nom Complet</label>
                   <input 
                    id="form-name"
                    type="text"
                    value={formData.name}
                ...
                <div className="space-y-2">
                   <label htmlFor="form-contact" className="text-[10px] font-black uppercase tracking-widest opacity-60">WhatsApp / Téléphone</label>
                   <input 
                    id="form-contact"
                    type="text"
                ...
                <div className="space-y-2">
                   <label htmlFor="form-subject" className="text-[10px] font-black uppercase tracking-widest opacity-60">Type de Demande</label>
                   <select 
                    id="form-subject"
                    value={formData.subject}
                    onChange={e => setFormData({...formData, subject: e.target.value})}
                    className="w-full p-6 bg-slate-900 border-b-2 border-white/20 outline-none font-bold text-xl focus:border-cyan-400 transition-all rounded-t-2xl appearance-none cursor-pointer"
                   >
                     <option value="Demande de devis forage">Devis Forage</option>
                ...
                <div className="space-y-2">
                   <label htmlFor="form-message" className="text-[10px] font-black uppercase tracking-widest opacity-60">Précisez votre besoin (Lieu, Type de sol...)</label>
                   <textarea 
                    id="form-message"
                    value={formData.message}
                    onChange={e => setFormData({...formData, message: e.target.value})}
                    className="w-full p-6 bg-white/5 border-b-2 border-white/20 outline-none font-bold text-xl focus:border-cyan-400 transition-all rounded-t-2xl h-32 resize-none" placeholder="EX: FORAGE À 150M À LOMÉ..." required></textarea>
                </div>
                <button type="submit" className="w-full bg-red-600 text-white font-black py-6 rounded-[2rem] uppercase tracking-widest text-lg hover:bg-white hover:text-red-600 transition-all shadow-xl flex items-center justify-center gap-4 active:scale-95">
                  Lancer l'expertise <Send size={24} />
                </button>
              </form>
            </ScrollReveal>
          </div>
      ...
      <a href={WHATSAPP_LINK} target="_blank" className="fixed bottom-10 right-10 z-[100] group">
         <div className="bg-[#25D366] text-white p-6 rounded-full shadow-[0_20px_60px_rgba(37,211,102,0.4)] group-hover:scale-110 transition-transform flex items-center justify-center relative active:scale-95">
            ...
            <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-600 rounded-full flex items-center justify-center text-[10px] font-black animate-pulse">1</span>
         </div>
      </a>
    </div>
  );
